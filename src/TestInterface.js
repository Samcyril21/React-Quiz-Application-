...
export default function TestInterface() {
  ...
  // Dynamic way
  let ActiveQuestionArr = Array(NoOfButtons)
    .fill(NotActive)
    .map((ele, index) => {
      if (index === 0) {
        return YesActive;
      ...
      <React.Fragment>
        <div id="TestScreen" style={{ backgroundColor: '#fff' }}>
          <div id="Interface_Top">
            <div id="testheading">
              <h2 className="Pagecolor1">
                <span className="Pagecolor1">Java</span>Script
                <span className="Pagecolor"> MCQ</span>
              </h2>
            </div>
            <div id="nameandcounter">
              <h4>Name : {Name}</h4>
              <h3>
          ...
          <div id="Qpartandmap">
            ...
            <button disabled={BtnDisabled[19]} onClick={()=>goto(19)} style={ActiveQuestion[19]}> 20 </button> */}
              ...
              <button onClick={SubmitTest} id="leftsideSubmit">
                {' '}
                Submit{' '}
              </button>
            </div>
            ...
            <div id="Qpart">
              <div>
                <p>
                  {qno + 1}. {QuestionsArr[qno].question}
                </p>
                <div className="optionscontainer">
                  {QuestionsArr[qno].options.map((Option, index) => {
                    return (
                      <div id="optionsdiv" key={index}>
                        <label>
                          <input
                            type="radio"
                            name="Options"
                            value={Option}
                            checked={SelectedAnswers[qno] === Option}
                            onChange={() => HandleSelection(Option, qno)}
                          />
                ...
                <div id="PreNextbuttonscontainer">
                  {qno > 0 ? (
                    <button id="discur" onClick={prevquestion}>
                      &#10096;&#10096; Prev
                    </button>
                  ) : (
                    <button id="discur" disabled="true" onClick={() => goto(1)}>
                      &#10096;&#10096; Prev
                    </button>
                  )}
                  {qno == QuestionsArr.length - 1 ? (
                    <button id="submitcenter" onClick={SubmitTest}>
                      Submit
                    </button>
                  ) : (
                    ''
                  ...
                  {qno < QuestionsArr.length - 1 ? (
                    <button id="discur" onClick={() => nextquestion(qno)}>
                      Next &#10097;&#10097;
                    </button>
                  ) : (
                    <button disabled="true" id="discur">
                      Next &#10097;&#10097;
                    </button>
                  )}
                </div>
    ...
    return (
      <div id="result">
        <>
          ...
          <h3>
            You Attempted {SelectedAnswers.length} out of {QuestionsArr.length}{' '}
            Questions
          </h3>
          <h3>
            Your Marks : {Mark}/{QuestionsArr.length}
          </h3>
  ...
  return <div>{TimerANDTestON ? Result() : Questions()}</div>;
}
